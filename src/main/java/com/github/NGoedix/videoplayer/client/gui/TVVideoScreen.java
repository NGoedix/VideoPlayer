...
public class TVVideoScreen extends Screen {
    ...
    public TVVideoScreen(BlockEntity be, String url, int tick, int volume, boolean loop) {
        super(new TranslatableText("gui.tv_video_screen.title"));
        this.be = be;
        this.url = url;
        this.tick = tick;
        this.volume = volume;
    ...
    protected void init() {
        ...
        addDrawableChild(urlBox = new TextFieldWidget(textRenderer, leftPos + 10, topPos + 30, imageWidth - 26, 20, new LiteralText("")));
    ...
    public void render(MatrixStack pPoseStack, int pMouseX, int pMouseY, float pPartialTick) {
        ...
        textRenderer.draw(pPoseStack, new TranslatableText("gui.tv_video_screen.url_text"), width / 2 - textRenderer.getWidth(new TranslatableText("gui.tv_video_screen.url_text")) / 2, topPos + 16, 0xFFFFFF);
    }
