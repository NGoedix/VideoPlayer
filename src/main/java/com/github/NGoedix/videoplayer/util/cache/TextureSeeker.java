...
public class TextureSeeker extends Thread {
    ...
    public static final Logger LOGGER = LogManager.getLogger(VideoPlayer.class);
    ...
    public void run() {
        ...
        try {
            byte[] data = load(cache.url != null ? cache.url : "");
            ...
            try {
                ...
                if (type != null && type.equalsIgnoreCase("gif")) {
                    ...
                    if (status == GifDecoder.STATUS_OK) {
                        mc.submitAndJoin(() -> cache.process(gif));
                        processed = true;
                    } else {
                    ...
                    try {
                        ...
                        if (image != null) {
                            mc.submitAndJoin(() -> cache.process(image));
                            processed = true;
                        }
