...
public class SendCustomVideoMessage extends AbstractClientPacket<SendCustomVideoMessage> {
    ...
    protected void handleClientSide() {
        if (this.state == VideoMessageType.START) {
            ...
            if (mode == 0) ClientHandler.openVideo(this.url, this.volume, this.isControlBlocked, this.canSkip, this.optionInMode, this.optionInSecs, this.optionOutMode, this.optionOutSecs);
